...
abstract contract BorrowUtils is Base {
    function getCurrentOwed(
        ...
        return
            owed.mulDiv(
                ...
                vaultStorage.users[account].interestAccumulator
            );
    }
    ...
    function getCurrentOwed(
        ...
        return
            getCurrentOwed(
                vaultCache,
                account,
                vaultStorage.users[account].getOwed()
            );
    }

    function loadUserBorrow(
        VaultCache memory vaultCache,
        address account
    ) private view returns (Owed newOwed, Owed prevOwed) {
        //@note
        //Follow-up
        //  getOwed()?
    ...
    function setUserBorrow(
        ...
        //@note
        //Intention
        //  1) Update user's owed amount
        //  2) Update user's interest accumulator
    ...
    function increaseBorrow(
        ...
        //@note
        //Intention
        //  1) Load user's current owed amount
        //  2) Update user's debt
        //  3) Update total borrows
        //  4) Log the borrow
        //Follow-up
        //  4) ?
    ...
    function decreaseBorrow(
        ...
        //  1) Load user's current owed amount
        //  2) Update user's debt
        //      Ensure don't repay mor than owed amount
        //  3) Update total borrows
        //      if totalBorrows > user's owed amount -> totalBorrows = totalBorrows - owedExact + owedRemaining
        //      else                                 -> totalBorrows = owedRemaining
        //          -> Because of rounding up user's debt
        //  4) Log the repay
        ...
        //3
        vaultStorage.totalBorrows = vaultCache.totalBorrows = vaultCache
            .totalBorrows > owedExact
            ? vaultCache.totalBorrows.subUnchecked(owedExact).addUnchecked(
                owedRemaining
            )
            : owedRemaining;
    ...
    function transferBorrow(
        ...
        Assets repayAssets = fromPrevAssets > assets + fromAssets
            ? fromPrevAssets.subUnchecked(fromAssets)
            : assets;
        ...
        Assets borrowAssets = assets + toPrevAssets > toAssets
            ? toAssets.subUnchecked(toPrevAssets)
            : assets;
    ...
    function calculateDTokenAddress()
        internal
        view
        virtual
    ...
    function logBorrow(
        ...
        if (!interest.isZero())
            emit InterestAccrued(account, interest.toUint());
