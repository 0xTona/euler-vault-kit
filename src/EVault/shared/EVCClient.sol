...
abstract contract EVCClient is Storage, Events, Errors {
    ...
    modifier onlyEVCChecks() {
        //@note
        //Intention
        //  Call by EVC && checks are in progress
    ...
    // deferred
    function EVCAuthenticateDeferred(
        bool checkController
    ) internal view virtual returns (address) {
        //@note
        //Intention
        //  Get `EVC.EC.onBehalfOfAccount`
        //      Called only by EVC
        //  If checkController -> Ensure `address(this)` is the controller of the current `EVC.EC.onBehalfOfAccount`

        assert(msg.sender == address(evc)); // this ensures that callThroughEVC modifier was utilized

        (address onBehalfOfAccount, bool controllerEnabled) = evc
            .getCurrentOnBehalfOfAccount(
                checkController ? address(this) : address(0)
            );

        if (checkController && !controllerEnabled)
            revert E_ControllerDisabled();
    ...
    function EVCAuthenticate() internal view virtual returns (address) {
        ...
        //Intention
        //  Get the caller
        //      If call by EVC -> EVC.EC.onBehalfOfAccount
        //      If not         -> msg.sender
    ...
    function EVCAuthenticateGovernor() internal view virtual returns (address) {
        if (msg.sender == address(evc)) {
            ...
            if (
                isKnownNonOwnerAccount(onBehalfOfAccount) ||
    ...
    function isKnownNonOwnerAccount(
        address account
    ...
    function EVCRequireStatusChecks(address account) internal virtual {
        //@note
        //Intention
        //  account == CHECKACCOUNT_NONE -> Vault status check
        //  account != CHECKACCOUNT_NONE -> Account + Vault status check
    ...
    function enforceCollateralTransfer(
        address collateral,
        uint256 amount,
        address from,
        address receiver
    ...
    function hasAnyControllerEnabled(
        address account
    ) internal view returns (bool) {
